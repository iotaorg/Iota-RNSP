...
while ( my $row = $csv->getline($fh) ) {
    ...
    if ( !$header_found ) {
        ...
        if (
                exists $registro->{qtde_indicadores_preenchido_ou_justificado}
            ) {

            $registro->{qtde_indicadores_justificado} =
                $registro->{qtde_indicadores_preenchido_ou_justificado} -
                $registro->{qtde_indicadores_preenchido};

            $registro->{nome_eixo} = $registro->{nome_eixo}.  ' (' . $registro->{axis_id} . ')';

            $eixos->{$registro->{nome_eixo}} = 1;

            my $key = join '|', $registro->{nome_uf}, $registro->{nome_cidade}, $registro->{user_id}, $registro->{programa_de_metas};

            $rot->{$_}{$key}{$registro->{nome_eixo}} = $registro->{$_} for qw/qtde_indicadores_justificado qtde_indicadores_preenchido_ou_justificado qtde_indicadores_preenchido/;
