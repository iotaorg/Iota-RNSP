...
eval {
    $schema->txn_do(
        sub {
            ...
            # sem valor 2
            ( $res, $c ) =
              ctx_request( POST $variable_url, [ 'variable.value.create.value_of_date' => '2022-12-22 14:22:44', ] );
            ok( !$res->is_success, 'variable value not created' );
            is( $res->code, 400, 'value added -- 400 ' );
            ...
            ok( my $updated_var = $schema->resultset('VariableValue')->find( { id => $variable->{id} } ), 'var in DB' );
