			...
			$.each(periods, function(period_index, period){
				...
				$.each(indicadores, function(i,item){
					...
					if (item.valores.length > 0){
						...
						table_content += "<td class='grafico'><a href='$$url'><canvas id='graph-$$id' width='40' height='20'></canvas></a></td>".render({id: cont, url: (base_url) ? (base_url + "/" + item.name_url) : ((window.location.href.slice(-1) == "/") ? item.name_url : window.location.href + "/" + item.name_url)});
