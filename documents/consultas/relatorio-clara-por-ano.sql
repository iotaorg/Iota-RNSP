CREATE OR REPLACE FUNCTION public.func_status_indicadores_by_user_justificado_by_year(
    abc integer,
    eixo integer,
...
(   SELECT
       ...
       (CASE WHEN i.indicator_type = 'varied' THEN
            COALESCE((SELECT MAX(x.casda) FROM (
             ...
             WHERE  a.indicator_id = i.id AND a.user_id = abc AND region_id IS NULL AND a.valid_from = in_this_date
             GROUP BY 1
            ) x)
            ,0)
       ELSE
    ...
    FROM indicator i
    LEFT JOIN indicator_value iv_any ON iv_any.user_id = abc AND iv_any.indicator_id = i.id AND iv_any.region_id IS NULL AND iv_any.valid_from = in_this_date
...
select
    ...
    func_status_indicadores_by_user_by_year(u.id, a.id,
        array(
    ...
    func_status_indicadores_by_user_justificado_by_year(u.id, a.id,
        array(
...
copy _saida to '/tmp/status.indicadores.por-ano.csv' CSV HEADER;