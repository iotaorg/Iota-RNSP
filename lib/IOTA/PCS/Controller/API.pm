...
A API do IOTA::PCS é uma API primariamente desenvolvida utilizando RESTful HTTP. Para realizar uma ação são feitas requisições HTTP a determinados endpoints utilizando alguns parâmetros e recebendo em resposta um conteúdo formatado.
...
package IOTA::PCS::Controller::API;
...
sub api_key_check : Private {
    ...
    unless (ref $c->user eq 'IOTA::PCS::TestOnly::Mock::AuthUser'){
