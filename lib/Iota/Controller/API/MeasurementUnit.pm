...
sub measurement_unit_GET {
    ...
    $self->status_ok( $c, entity => { ( map { $_ => $object_ref->{$_} } qw(name short_name id user_id created_at) ) } );
}
...
sub measurement_unit_POST {
    ...
    $self->status_accepted(
        $c,
        location => $c->uri_for( $self->action_for('measurement_unit'), [ $obj->id ] )->as_string,
...
sub list_GET {
    ...
    $self->status_ok( $c, entity => { measurement_units => \@objs } );
}
