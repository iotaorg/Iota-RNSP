package Iota::Controller::API::User::IndicatorConfig;
...
sub indicator_config_GET {
    ...
    $self->status_ok( $c, entity => { ( map { $_ => $object_ref->{$_} } qw(technical_information indicator_id) ) } );
}
...
sub indicator_config_POST {
    ...
    $self->status_accepted(
        $c,
        location =>
          $c->uri_for( $self->action_for('indicator_config'), [ $c->stash->{user}->id, $obj->id ] )->as_string,
        entity => { id => $obj->id }
      ),
...
sub list_POST {
    ...
    $self->status_created(
        $c,
        location =>
          $c->uri_for( $self->action_for('indicator_config'), [ $c->stash->{user}->id, $object->id ] )->as_string,
        entity => { id => $object->id }
    );
...
sub list_GET {
    ...
    $self->status_ok( $c, entity => { ( map { $_ => $config->{$_} } qw(id technical_information indicator_id) ) } );
}
