package Iota::Controller::API::Indicator::Variations;
...
sub variation_GET {
    ...
    $self->status_ok( $c, entity => { ( map { $_ => $object_ref->{$_} } qw(indicator_id name order) ) } );
}
...
sub variation_DELETE {
    ...
    if ( $c->user->id == $obj->indicator_id || $c->check_any_user_role(qw(admin superadmin)) ) {
        ...
        $c->model('DB::IndicatorVariablesVariationsValue')->search( { indicator_variation_id => $obj->id } )->delete;
...
sub list_GET {
    ...
    if ( $c->stash->{indicator}->indicator_type eq 'varied' ) {
        ...
        $where->{user_id} = [ $c->stash->{indicator}->user_id, $c->stash->{user_id} || $c->user->id ];
    ...
    foreach my $obj (@list) {
        push @objs, {
            ...
            url => $c->uri_for_action( $self->action_for('variation'), [ $c->stash->{indicator}->id, $obj->{id} ] )
              ->as_string,
        };
    }

    $self->status_ok( $c, entity => { variations => \@objs } );
}
...
sub list_POST {
    ...
    $self->status_created(
        $c,
        location =>
          $c->uri_for( $self->action_for('variation'), [ $c->stash->{indicator}->id, $object->id ] )->as_string,
        entity => { id => $object->id }
    );
