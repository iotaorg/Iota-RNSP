...
sub _get_values_periods {
    ...
    while ( my $row = $rs->next ) {
        ...
        $out->{ $row->{user_id} }{ $row->{valid_from} }{ $row->{variable_id} } = [ $row->{value}, $row->{source}, ];
    }
...
sub _get_values_variation {
    ...
    while ( my $row = $variations_rs->next ) {
        ...
        foreach my $val ( @{ $row->{indicator_variables_variations_values} } ) {
            ...
            $out->{ $val->{user_id} }{ $val->{indicator_variables_variation_id} }{ $row->{name} }{ $val->{valid_from} }
              = $val->{value};
            ...
            foreach my $date ( keys %{ $params{values}{$user_id} } ) {
                ...
                if ( $indicator->indicator_type eq 'varied' ) {
                    ...
                    foreach my $variation ( keys %$filled_variations ) {
                        ...
                        $out->{$user_id}{ $indicator->id }{$date}{$variation} = [ $valor, [ keys %sources ] ];
                    }
                    ...
                    # '' = variacao
                    $out->{$user_id}{ $indicator->id }{$date}{''} = [ $valor, [ keys %sources ] ];
                }
