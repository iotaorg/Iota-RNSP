...
from (
    ...
    from indicators i
    left join indicator_value iv_last on iv_last.user_id = ? and iv_last.indicator_id = i.id AND iv_last.valid_from = last_period AND iv_last.region_id is null
    left join indicator_value iv_any on iv_any.user_id = ? and iv_any.indicator_id = i.id  AND iv_any.region_id is null
