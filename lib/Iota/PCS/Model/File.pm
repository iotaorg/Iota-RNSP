package Iota::PCS::Model::File;
use Moose;
use utf8;
use JSON qw/encode_json/;

use Iota::PCS::Model::File::XLSX;
use Iota::PCS::Model::File::XLS;
use Iota::PCS::Model::File::CSV;
...
sub process {
    ...
    eval{
        if ($upload->filename =~ /xlsx$/){
            $parse = Iota::PCS::Model::File::XLSX->new->parse( $upload->tempname );
        }elsif ($upload->filename =~ /xls$/){
            $parse = Iota::PCS::Model::File::XLS->new->parse( $upload->tempname );
        }elsif ($upload->filename =~ /csv$/){
            $parse = Iota::PCS::Model::File::CSV->new->parse( $upload->tempname );
        }
    };
    die $@ if $@;
