...
# network CSV
sub pref_dados_csv : Chained('/network_object') : PathPart('variaveis.csv') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
    $c->stash->{type} = 'csv';
}

sub pref_dados_csv_check: Chained('/network_object') : PathPart('variaveis.csv.checksum') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
...
# network XML
sub pref_dados_xml : Chained('/network_object') : PathPart('variaveis.xml') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
    $c->stash->{type} = 'xml';
}

sub pref_dados_xml_check: Chained('/network_object') : PathPart('variaveis.xml.checksum') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
...
# network JSON
sub pref_dados_json : Chained('/network_object') : PathPart('variaveis.json') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
    $c->stash->{type} = 'json';
}

sub pref_dados_json_check: Chained('/network_object') : PathPart('variaveis.json.checksum') : CaptureArgs(0) {
    my ( $self, $c ) = @_;
